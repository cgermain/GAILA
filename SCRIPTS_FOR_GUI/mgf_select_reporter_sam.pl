...
# KEY: THE FIRST ENTRY IS WHERE YOU READ FROM, THE SECOND IS GOING TO BE THE ONE YOU
# WRITE TO. WE'LL ALSO BE MAKING TEMPORARY FILES SOMEWHERE BUT THAT'S UNRELATED TO
# WHERE WE WRITE THE FINAL FILES. THAT'LL PROBABLY BE SOMEWHERE THAT'S HIDDEN, LIKE
# IN ANOTHER FOLDER.
# THE THIRD IS TEMP_DIR
# THE FOURTH PARAMETER IS THE MZ_ERROR
# THE FIFTH PARAMETER IS THE TYPE (eg:TMT10)
# THE SIXTH IS MIN INTENSITY
# THE SEVENTH IS MIN REPORTERS
...
# 					# open (OUT_NOT,">$dir/not-selected-d$mz_error-$type-$min_intensity-$min_reporters/$filename");
# 					# open (OUT_CAL,">$dir/selected-d$mz_error-$type-$min_intensity-$min_reporters/reporter_cal/$filename");
# 					if(open (OUT,">$dir/selected-d$mz_error-$type-$min_intensity-$min_reporters/mgf-files/$filename"))
# 					{
# 						open (OUT_TABLE,">$dir/selected-d$mz_error-$type-$min_intensity-$min_reporters/mgf-txt-files/$filename.txt");
# 						print OUT_TABLE qq!filename\tscan\tcharge\trt\tMS1_intensity!;
# 						foreach my $reporter (@reporters)
# 						{
# 							my $reporter_=int($reporter);
...
# 		print "\n$count_spectra spectra selected ($mz_error, $type, $min_intensity, $min_reporters) out of $count_all_spectra in \"merged\".\n";
# 	}
# 	# print "Deleting not-selected and merged directories.\n";
# 	# rmtree(qq!$dir/not-selected-d$mz_error-$type-$min_intensity-$min_reporters!);
# 	# rmtree(qq!$dir/merged-d$mz_error-$type-$min_intensity-$min_reporters!);
# }
